...
const SuggestComparisonFlat = ({ images, prompts, existingCriteria, handleSuggestionButtonClick, messageApi, llmConfig = {} }) => {
    ...
    const _handleSuggestionButtonClick2 = () => {
        ...
        const suggestion = {
            ...
            candidateValues: suggestionMetaData.candidateValues,
            explanations: suggestionMetaData.explanations
        };
        handleSuggestionButtonClick(suggestion, 'add-criterion');
        if (messageApi) {
    ...
    const generateInitialKeywords = (retryCount = 0) => {
        ...
        // For Mode C, we create a simplified schema structure for keyword generation
        const simplifiedSchema = {};
        if (existingCriteria && existingCriteria.length > 0) {
            existingCriteria.forEach(criterion => {
                simplifiedSchema[criterion] = {
                    _nodeType: 'attribute'
                };
            });
        }
                ...
                {Object.keys(suggestionMetaData).length === 0 ? <div>Loading...</div> :
                    <>
                        ...
                        <div className="suggestion-preview">
                            <div> <b><i> Add criterion: </i></b> </div>
                            <div className='node'> {suggestionMetaData.criterionName} </div>
                        </div>
...
export default SuggestComparisonFlat;